...
const highlight = (context: vscode.ExtensionContext) => {
  ...
  // Remove highlights
  let removeHighlight = vscode.commands.registerCommand(
    "code-save.removeHighlight",
    async () => {
      const activeEditor = vscode.window.activeTextEditor;
      ...
      parsedSavedRangeArray.forEach((singleRangeString: string) => {
        ...
        let cursorPositionOffset =
          activeEditor?.document.offsetAt(cursorPosition);

        // Start and End selected position
        let rangeStartPosition = new vscode.Position(
          singleRange.startLine,
          singleRange.startCharacter
        );
        let rangeEndPosition = new vscode.Position(
          singleRange.endLine,
          singleRange.endCharacter
        );

        let rangeStartPositionOffset =
          activeEditor?.document.offsetAt(rangeStartPosition);
        let rangeEndPositionOffset =
          activeEditor?.document.offsetAt(rangeEndPosition);

        if (
          cursorPositionOffset &&
          rangeStartPositionOffset &&
          rangeEndPositionOffset &&
          cursorPositionOffset >= rangeStartPositionOffset &&
          cursorPositionOffset <= rangeEndPositionOffset
        ) {
          index = i;
        }
        i = i + 1;
      ...
      await context.workspaceState.update(
        hashedFilePath,
        JSON.stringify(parsedSavedRangeArray)
      );
  ...
  let removeAllHighlight = vscode.commands.registerCommand(
    "code-highlight.removeAllHighlights",
    async () => {
      const activeEditor = vscode.window.activeTextEditor;
      let filePath = activeEditor?.document.uri.fsPath;
      let hashedFilePath = crypto
