...
function getSavePath(
  urlString: readonly vscode.WorkspaceFolder[],
  fileName: string
): vscode.Uri {
  let pathArray = urlString[0].uri.path;
  let projectPath = pathArray.split("/");
  let projectName = projectPath?.pop();
  let pathUri = vscode.Uri.file(
    `${projectPath.join("/")}/${projectName}-codesave/${fileName}.txt`
  );
    ...
    if (activeEditor) {
      ...
      if (fileName) {
        ...
        if (fs.existsSync(getSavePath(workspacePath, fileName).path)) {
          let savedFile = await vscode.workspace.fs
            .readFile(getSavePath(workspacePath, fileName))
            .then((data) => {
        ...
        vscode.workspace.fs.writeFile(
          getSavePath(workspacePath, fileName),
          writeData
        );
